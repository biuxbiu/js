...
`Javascript` 有一条主线程同步执行任务，遇到异步方法的时候进入异步任务列表。异步列表任务必须得等到主线程执行任务完之后方可执行任务。
...
两者 `<script>` 标签属性，都需要通过外链 `Javascript` 文件访问。<br>
两者都为异步加载；

**async**：异步加载完后立马执行；<br>
**defer**：异步加载完后延迟执行；（等待页面元素渲染完成后执行）
